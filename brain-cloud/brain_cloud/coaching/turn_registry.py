"""Turn registry for the 8-turn Clarity Session.

Linear progression (OB-D-006): Turn 1 → 2 → ... → 8. No branching.
...
# The 8 turns are a static registry. No database lookup needed.
...
class TurnRegistry:
    """Linear 8-turn Clarity Session progression."""

    def get_turn(self, turn_number: int) -> TurnSpec:
        """Return the spec for a given turn number (1-8).

        Raises KeyError if turn_number is outside 1-8.
        """
        if turn_number not in _TURNS:
            raise KeyError(f"Turn {turn_number} does not exist (valid: 1-8)")
    ...
    def is_complete(self, turn_number: int) -> bool:
        """Returns True if turn_number > 8 (session finished)."""
