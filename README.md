...
| **Brain Cloud process** | Launched as a subprocess inside Electron — one command starts everything. | Standalone service with its own lifecycle. Multiple AI clients connect independently. | Zero-friction demo: `npm run electron:dev` and you're coaching in 30 seconds. Production requires Brain Cloud running independently so any MCP-compatible client (Claude Code, other AI models) can connect whether or not the desktop app is open. |
